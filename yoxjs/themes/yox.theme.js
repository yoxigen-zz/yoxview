...
yox.theme.prototype = {
    ...
    init: function(container, options, yoxId){
        ...
        for(var moduleName in modulesConfig){
			...
			for(var i=moduleElements.length - 1, moduleElement; moduleElement = moduleElements[i]; i--){
                elementYoxId = moduleElement.getAttribute("data-yoxid");
				if (elementYoxId && elementYoxId != yoxId)
					moduleElements.splice(i, 1);
				else
					moduleElement.setAttribute("data-yoxid", yoxId);
			}
